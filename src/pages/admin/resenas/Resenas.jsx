...
export default function Resenas() {
  ...
  const cargar = () => api.listarResenas()
          ...
          {filtradas.map((r) => (
            <div key={r.id_resena} style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #f0f0f0', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              ...
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', marginBottom: 10 }}>
                ...
                {r.calificacion_facilidad_pedido && <div style={{display:'flex',alignItems:'center',gap:4}}><span style={{ fontSize: 12, color: '#888' }}>Facilidad web: </span><strong style={{ fontSize: 13, display:'flex', alignItems:'center', gap:3 }}>{{muy_facil: <><CheckCircle size={12} color="#16a34a"/>Muy fácil</>, algo_confuso: <><HelpCircle size={12} color="#f59e0b"/>Algo confuso</>, dificil: <><XCircle size={12} color="#CA0B0B"/>Difícil</>}[r.calificacion_facilidad_pedido] || r.calificacion_facilidad_pedido}</strong></div>}
