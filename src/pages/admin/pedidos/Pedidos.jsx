...
// Pedidos muestra todo excepto 'entregado' -- ese estado ya generó plata real
// y pertenece a la pantalla Ventas, así que no tiene sentido ofrecerlo en el
// filtro de esta pantalla.
const ESTADOS_PEDIDOS = ESTADOS.filter((e) => e !== 'entregado');
...
function ModalDetalle({ open, onClose, venta }) {
  ...
  const wppMsg   = encodeURIComponent(`Hola ${venta.cliente} 👋, tu pedido #${venta.id_venta} de ChocoFreseo ya está confirmado y en preparación 🍫, en pocos minutos será despachado hacia tu ubicación 🛵, te agradecemos estar atento/a para recibirlo.\n\nCuando recibas tus productos, te invitamos a llenar este pequeño formulario, tu opinión es muy importante para nosotros ⭐:\nchocofreseo.com/landing#reseñas`);
...
export default function Pedidos() {
  ...
  const filtrados = lista.filter((v) => {
    if (v.estado === 'entregado') return false; // entregados viven en la pantalla Ventas
        ...
        return (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 16 }}>
            ...
            <select style={estiloFiltro} value={filtroEstado} onChange={(e) => setFiltroEstado(e.target.value)}>
              ...
              {ESTADOS_PEDIDOS.map((e) => <option key={e} value={e}>{ESTADO_LABELS[e]}</option>)}
            </select>
                  ...
                  <tr key={v.id_venta}>
                    ...
                    <td data-label="Acciones">
                      <div className="acciones">
                        ...
                        {tienePermiso('anular_venta') && v.estado !== 'anulado' && v.estado !== 'despachado' && (
